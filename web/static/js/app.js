...
document.addEventListener('DOMContentLoaded', () => {
  ...
  ws.on('DICE_RESULT', ({ roller, expression, result, breakdown, is_verbose, context, dice_entry }) => {
    if (combatLog) {
      // Build the display string similar to v0.3.0 behavior
      const displayText = context
        ? context.replace(`[[${expression}]]`, is_verbose ? breakdown : result)
        : `${roller} rolls ${expression}: ${is_verbose ? breakdown : result}`;
      combatLog.addEntry(displayText, 'info');
    }
